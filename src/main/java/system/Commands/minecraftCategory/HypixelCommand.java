...
public class HypixelCommand implements Command {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        WebUtils.ins.getJSONObject("https://api.mojang.com/users/profiles/minecraft/" + args.get(0)).async((mojang) -> {
            ...
            msg.editMessage(new MessageUtils(":successful: Successfully, User was found").EmojisHolder())
            ...
            WebUtils.ins.getJSONObject("https://api.minetools.eu/profile/" + uuid).async((minecraft) -> {
                ...
                WebUtils.ins.getJSONObject("https://api.hypixel.net/player?key=" + APIKEY + "&uuid=" + uuid).async((hypixel) -> WebUtils.ins.getJSONObject("https://api.hypixel.net/guild?key=" + APIKEY + "&player=" + uuid).async((guild) -> {
                    WebUtils.ins.getJSONObject("https://api.hypixel.net/status?key=" + APIKEY + "&uuid=" + uuid).async((status) -> {
                        msg.editMessage(new MessageUtils(":successful: **User was found in Hypixel Network** now we are getting data :loading:").EmojisHolder()).queue();
                        ...
                        FIRSTJOIN = hypixel.get("player").get("firstLogin").asLong();
                        ...
                        this.guild = guild.get("guild").path("name").asText();
                        ...
                        if (hypixel.get("player").path("housingMeta").path("firstHouseJoinMs").asLong() == 0) {
                        ...
                        embed.addField("**__Status__**", getStatus, false);
                        ...
                        if (hypixel.get("player").path("socialMedia").path("links").path("DISCORD").asText().isEmpty()) {
                        ...
                        msg.editMessage("<@" + event.getAuthor().getId() + "> **Checking is done**").queue();
                        msg.editMessage(embed.build()).queue();
                    }, (error) -> msg.editMessage(new MessageUtils(":error: - `this users doesn't longer in hypixel network`").EmojisHolder()).queue());
                }));
            });
        }, (error) -> {
            msg.editMessage(new MessageUtils(":error: - **Something went wrong with this user**").EmojisHolder()).queue();
        });
    }
